...
const Genres = styled.div`
  display: grid;
  width: 100%;
  grid-template-columns: repeat(6, minmax(60px, 150px));
  /* justify-self: start; */
  align-items: center;
  /* text-align: left; */
`;
    ...
    <>
      ...
      >
        <DetailUL>
          ...
          <DetailLI key={data.backdrop_path}>
            <Img src={makeBgPath(data.backdrop_path)}></Img>
          </DetailLI>
          <DetailLI key={data.original_title}>
            <Title>{data.original_title}</Title>
          </DetailLI>
          <DetailLI key={data.id}>{data.overview.substr(0, 150)}...</DetailLI>
          <DetailLI key={data.release_date}>
